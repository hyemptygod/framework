    ...
    internal class AssetBundleBuildTab
    {
        ...
        internal void OnGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(!AssetBundleModel.Model.DataSource.CanSpecifyBuildTarget))
            {
                ...
                {
                    tgt = (ValidBuildTarget)EditorUserBuildSettings.activeBuildTarget;
                }
                if (tgt != m_UserData.m_BuildTarget)
                {
                    ...
                    if (m_UserData.m_UseDefaultPath)
                    {
                        m_UserData.m_OutputPath = "Assets/StreamingAssets/AssetBundles/";
                        m_UserData.m_OutputPath += PathConst.GetPlatformName((UnityEditor.BuildTarget)tgt);
            ...
            using (new EditorGUI.DisabledScope(!AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory))
            {
                ...
                EditorGUILayout.TextField("Output Path", m_UserData.m_OutputPath);
        ...
        private void ResetPathToDefault()
        {
            m_UserData.m_UseDefaultPath = true;
            m_UserData.m_OutputPath = "Assets/StreamingAssets/AssetBundles/";
            m_UserData.m_OutputPath += PathConst.GetPlatformName((UnityEditor.BuildTarget)m_UserData.m_BuildTarget);
            //EditorUserBuildSettings.SetPlatformSettings(EditorUserBuildSettings.activeBuildTarget.ToString(), "AssetBundleOutputPath", m_OutputPath);
        }

        private void CopyDll()
        {
            var source = Application.streamingAssetsPath + "/dll/";
            var dest = m_UserData.m_OutputPath + "/dll/";
            ...
            foreach (var f in Directory.GetFiles(source))
            {
                if (f.EndsWith(".meta"))
                    continue;
                File.Copy(f, f.Replace(source, dest));
            }
            AssetDatabase.Refresh(ImportAssetOptions.ForceUpdate);


            EditorApplication.delayCall += WriteVersion;
        }

        private void WriteVersion()
        {
            Framework.Start.AssetVersionHelper.CreateUpdateConfig(m_UserData.m_OutputPath, FrameworkEditor.EditorUtil.GetFiles(m_UserData.m_OutputPath));
