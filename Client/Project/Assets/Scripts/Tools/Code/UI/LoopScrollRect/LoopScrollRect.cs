    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public void ClearCells()
        {
            if (Application.isPlaying)
            {
                ...
                {
                    returnObjectDelegate(content.GetChild(i));
                }
            }
        }
        ...
        private float ResetItemPosition(bool end, int offset = 0)
        {
            StopMovement();

            itemTypeEnd = end == reverseDirection ? offset : totalCount - offset;
            itemTypeStart = itemTypeEnd;

            if (totalCount >= 0 && itemTypeStart % constraintCount != 0)
                Debug.LogWarning("Grid will become strange since we can't fill items in the last line");

            for (int i = m_Content.childCount - 1; i >= 0; i--)
            {
                returnObjectDelegate(m_Content.GetChild(i));
            }

            if (directionSign == -1)
                return viewRect.rect.size.y;
            else
                return viewRect.rect.size.x;
        }

        private void ResetContentPosition(bool end, float sizeFilled, float sizeToFill)
        {
            Vector2 pos = m_Content.anchoredPosition;
            float dist = end ? Mathf.Max(0, sizeFilled - sizeToFill) : 0;
            if (reverseDirection)
                dist = -dist;
            if (directionSign == -1)
                pos.y = dist;
        ...
        /// 重新填充
        /// </summary>
        /// <param name="end">是否倒序</param>
        /// <param name="offset"></param>
        public void RefillCells(bool end, int offset = 0)
        {
            if (!Application.isPlaying)
                return;

            float sizeFilled = 0;
            float sizeToFill = ResetItemPosition(end, offset);
            while (sizeToFill > sizeFilled)
            {
                float size = reverseDirection == end ? NewItemAtEnd() : NewItemAtStart();
                if (size <= 0)
                    break;
                sizeFilled += size;
            }
            ResetContentPosition(end, sizeFilled, sizeToFill);
        }

        public IEnumerator<float> RefillCellsOneByOne(bool end, Action callback, float interval = 0f, int offset = 0)
        {
            if (!Application.isPlaying)
                yield break;
            float sizeFilled = 0;
            float sizeToFill = ResetItemPosition(end, offset);
            int count = 0;
            bool fromend = reverseDirection == end;
            while (sizeToFill > sizeFilled)
            {
                if (count == 0)
                    count = constraintCount - (fromend ? (content.childCount % constraintCount) : 0);
                float size = fromend ? NewItemAtEndOne() : NewItemAtStartOne();
                count--;
                if (size <= 0)
                    break;
                if (count == 0)
        ...
        protected float NewItemAtStartOne()
        {
            if (totalCount >= 0 && itemTypeStart - constraintCount < 0)
            {
                return 0;
        ...
        protected float NewItemAtEndOne()
        {
            ...
            var size = Mathf.Max(GetSize(newItem), 0);
            itemTypeEnd++;
        ...
        protected float NewItemAtEnd()
        {
            ...
            int count = constraintCount - (content.childCount % constraintCount);
        ...
        {
            if (((m_Dragging || m_Velocity != Vector2.zero) && totalCount >= 0 && itemTypeStart < constraintCount)
                || content.childCount == 0)
            {
                return 0;
            }
            ...
            for (int i = 0; i < constraintCount; i++)
            {
                ...
                itemTypeEnd--;
                if (itemTypeEnd % constraintCount == 0 || content.childCount == 0)
                {
        ...
        public Func<GameObject> getObjectDelegate;
        public Action<Transform> returnObjectDelegate;

        private RectTransform InstantiateNextItem(int itemIdx)
        {
            RectTransform nextItem = getObjectDelegate().GetComponent<RectTransform>();
