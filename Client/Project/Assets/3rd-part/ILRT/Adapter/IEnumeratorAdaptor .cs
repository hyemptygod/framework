...
public class IEnumeratorAdaptor : CrossBindingAdaptor
{
    ...
    public override Type[] BaseCLRTypes
    {
        get
        {
            return new Type[] { typeof(IEnumerator<float>), typeof(IEnumerator), typeof(IDisposable) };
        }
    }
    ...
    class Adaptor : IEnumerator<float>, IEnumerator, IDisposable, CrossBindingAdaptorType
    {
        ...
        IMethod m_getCurrentMethodFloat;
        bool m_getCurrentMethodGotFloat;
        float IEnumerator<float>.Current
        {
            get
            {
                if (!m_getCurrentMethodGotFloat)
                {
                    m_getCurrentMethodFloat = instance.Type.GetMethod("get_Current", 0);
                    if (m_getCurrentMethodFloat == null)
                    {
                        //如果实现的是System.Collections.IEnumerator接口，则用下面的读取方式
                        m_getCurrentMethodFloat = instance.Type.GetMethod("System.Collections.IEnumerator.get_Current", 0);
                    }
                    m_getCurrentMethodGotFloat = true;
                }
                if (m_getCurrentMethodFloat != null)
                    return (float)appdomain.Invoke(m_getCurrentMethodFloat, instance, null);
                return 0f;
            }
        }
